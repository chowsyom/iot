...
Schedule schedule[] = {{ -1, -1, 5, 7, 30, 0, RC5, 0xe88, 12},{ -1, -1, 5, 7, 30, 20, RC5, 0xe84, 12}, { -1, -1, -1, 7, 30, 30, RC5, 0xe86, 12},{ -1, -1, -1, 7, 30, 50, RC5, 0xe84, 12}};
...
void loop() {
  ...
  if (millis() - lastUpdateTime > uptimeInterval) {
    ...
    for (int i = 0; i < 2; i++) {
      if (schedule[i].week == date[3]&&schedule[i].hour == date[2]&&schedule[i].minute == date[1]&&schedule[i].second == date[0]) {
        for (int j = 0; j < 3; j++) {
